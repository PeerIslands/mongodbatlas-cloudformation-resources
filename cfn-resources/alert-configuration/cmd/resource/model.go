// Code generated by 'cfn generate', changes will be undone by the next invocation. DO NOT EDIT.
// Updates to this type are made my editing the schema file and executing the 'generate' command.
package resource

// Model is autogenerated from the json schema
type Model struct {
	AlertConfigId   *string                  `json:",omitempty"`
	ApiKeys         *ApiKeyDefinition        `json:",omitempty"`
	Created         *string                  `json:",omitempty"`
	Enabled         *bool                    `json:",omitempty"`
	EventTypeName   *string                  `json:",omitempty"`
	GroupId         *string                  `json:",omitempty"`
	Id              *string                  `json:",omitempty"`
	IncludeCount    *bool                    `json:",omitempty"`
	ItemsPerPage    *int                     `json:",omitempty"`
	Links           []Link                   `json:",omitempty"`
	Matchers        []ApiMatcherView         `json:",omitempty"`
	MetricThreshold *ApiMetricThresholdView  `json:",omitempty"`
	Notifications   []ApiNotificationView    `json:",omitempty"`
	PageNum         *int                     `json:",omitempty"`
	Results         []ApiAlertView           `json:",omitempty"`
	Threshold       *ApiIntegerThresholdView `json:",omitempty"`
	TotalCount      *float64                 `json:",omitempty"`
	TypeName        *string                  `json:",omitempty"`
	Updated         *string                  `json:",omitempty"`
}

// ApiKeyDefinition is autogenerated from the json schema
type ApiKeyDefinition struct {
	PrivateKey *string `json:",omitempty"`
	PublicKey  *string `json:",omitempty"`
}

// Link is autogenerated from the json schema
type Link struct {
	ApiKeys *ApiKeyDefinition `json:",omitempty"`
	Href    *string           `json:",omitempty"`
	Rel     *string           `json:",omitempty"`
}

// ApiMatcherView is autogenerated from the json schema
type ApiMatcherView struct {
	ApiKeys   *ApiKeyDefinition `json:",omitempty"`
	FieldName *string           `json:",omitempty"`
	Operator  *string           `json:",omitempty"`
	Value     *string           `json:",omitempty"`
}

// ApiMetricThresholdView is autogenerated from the json schema
type ApiMetricThresholdView struct {
	ApiKeys    *ApiKeyDefinition `json:",omitempty"`
	MetricName *string           `json:",omitempty"`
	Mode       *string           `json:",omitempty"`
	Operator   *string           `json:",omitempty"`
	Threshold  *float64          `json:",omitempty"`
	Units      *string           `json:",omitempty"`
}

// ApiNotificationView is autogenerated from the json schema
type ApiNotificationView struct {
	ApiKeys                  *ApiKeyDefinition `json:",omitempty"`
	ApiToken                 *string           `json:",omitempty"`
	ChannelName              *string           `json:",omitempty"`
	DatadogApiKey            *string           `json:",omitempty"`
	DatadogRegion            *string           `json:",omitempty"`
	DelayMin                 *float64          `json:",omitempty"`
	EmailAddress             *string           `json:",omitempty"`
	EmailEnabled             *bool             `json:",omitempty"`
	FlowName                 *string           `json:",omitempty"`
	FlowdockApiToken         *string           `json:",omitempty"`
	IntervalMin              *float64          `json:",omitempty"`
	MicrosoftTeamsWebhookUrl *string           `json:",omitempty"`
	MobileNumber             *string           `json:",omitempty"`
	NotificationToken        *string           `json:",omitempty"`
	OpsGenieApiKey           *string           `json:",omitempty"`
	OpsGenieRegion           *string           `json:",omitempty"`
	OrgName                  *string           `json:",omitempty"`
	Roles                    []string          `json:",omitempty"`
	RoomName                 *string           `json:",omitempty"`
	ServiceKey               *string           `json:",omitempty"`
	Severity                 *string           `json:",omitempty"`
	SmsEnabled               *bool             `json:",omitempty"`
	TeamId                   *string           `json:",omitempty"`
	TeamName                 *string           `json:",omitempty"`
	TypeName                 *string           `json:",omitempty"`
	Username                 *string           `json:",omitempty"`
	VictorOpsApiKey          *string           `json:",omitempty"`
	VictorOpsRoutingKey      *string           `json:",omitempty"`
	WebhookSecret            *string           `json:",omitempty"`
	WebhookUrl               *string           `json:",omitempty"`
}

// ApiAlertView is autogenerated from the json schema
type ApiAlertView struct {
	AcknowledgedUntil      *string           `json:",omitempty"`
	AcknowledgementComment *string           `json:",omitempty"`
	AcknowledgingUsername  *string           `json:",omitempty"`
	AlertConfigId          *string           `json:",omitempty"`
	ApiKeys                *ApiKeyDefinition `json:",omitempty"`
	ClusterName            *string           `json:",omitempty"`
	Created                *string           `json:",omitempty"`
	CurrentValue           *CurrentValue     `json:",omitempty"`
	EventTypeName          *string           `json:",omitempty"`
	GroupId                *string           `json:",omitempty"`
	HostnameAndPort        *string           `json:",omitempty"`
	Id                     *string           `json:",omitempty"`
	LastNotified           *string           `json:",omitempty"`
	Links                  []Link            `json:",omitempty"`
	MetricName             *string           `json:",omitempty"`
	ReplicaSetName         *string           `json:",omitempty"`
	Resolved               *string           `json:",omitempty"`
	Status                 *string           `json:",omitempty"`
	TypeName               *string           `json:",omitempty"`
	Updated                *string           `json:",omitempty"`
}

// CurrentValue is autogenerated from the json schema
type CurrentValue struct {
	ApiKeys *ApiKeyDefinition `json:",omitempty"`
	Number  *float64          `json:",omitempty"`
	Units   *string           `json:",omitempty"`
}

// ApiIntegerThresholdView is autogenerated from the json schema
type ApiIntegerThresholdView struct {
	ApiKeys   *ApiKeyDefinition `json:",omitempty"`
	Operator  *string           `json:",omitempty"`
	Threshold *int              `json:",omitempty"`
	Units     *string           `json:",omitempty"`
}
