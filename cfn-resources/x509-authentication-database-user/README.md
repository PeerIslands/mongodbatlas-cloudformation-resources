...
./test/x509authenticationdatabaseuser.create-sample-cfn-request.sh YourProjectName > test.request.json 
echo "Sample request:"
...
repo_root=$(git rev-parse --show-toplevel)
source <(${repo_root}/quickstart-mongodb-atlas/scripts/export-mongocli-config.py)
${repo_root}/quickstart-mongodb-atlas/scripts/launch-x-quickstart.sh ${repo_root}/cfn-resources/x509-authentication-database-user/test/x509authenticationdatabaseuser.sample-template.yaml SampleProject1 ParameterKey=MonthsUntilExpiration,ParameterValue=${MonthsUntilExpiration} ParameterKey=UserName,ParameterValue=${UserName} ParameterKey=CustomerX509,ParameterValue=${CustomerX509} ParameterKey=ProjectId,ParameterValue=${ProjectId}
```
