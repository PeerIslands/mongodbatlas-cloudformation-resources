...
# Test backward compatibility with InstanceName (inputs_4)
# NOTE: CFN primary identifier requires WorkspaceName, so we set both for test framework validation
# In handlers, WorkspaceName takes precedence if both are present (no mutual exclusivity validation in CFN)
jq --arg instance_name "$workspaceName" \
	--arg project_id "$projectId" \
	--arg profile "$profile" \
	'.Profile?|=$profile
   | .ProjectId?|=$project_id
   | .InstanceName?|=$instance_name
   | .WorkspaceName?|=$instance_name' \
